...
class String
  def to_number(result = 0, potision_begin = 0)
    ...
    KEY.reverse.each do |item|
      if self.index(item).present?
          result += get_number(self[potision_begin..self.index(item)-1]).to_i * Hash[KANA_TO_ROM.to_a.map{|m| m.reverse}][item].to_i # >0
  ...
  def get_number(string, result = 0)
    begin
      ...
      current = current * Hash[KANA_TO_ROM.to_a.map{|m| m.reverse}][string.slice!(1..1) ].to_i if (Hash[KANA_TO_ROM.to_a.map{|m| m.reverse}][string[1]].to_i % 10 == 0) && string[1].present? && current % 10 != 0
      (string.length == 1) ? (return (result + current.to_i)) : result += current.to_i
    end while string.slice!(0).present?
  end
end
class Numeric
  def to_kansuji(result = '', count=0)
  ...
  def kanji_to_demo(string, kan = '')
    begin
      ...
      kan += (string.length >= 5) ? ('一' + KANA_TO_ROM[(string[0].to_i * 10**(string.length-1))]) : (KANA_TO_ROM[(string[0].to_i * 10**(string.length-1))])
    end while string.slice!(0).present?
  end
end
